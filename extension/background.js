...
function handleUrl(tabId, url) {
  try {
    ...
    chrome.storage.local.get({ globalTrusted: [], userTrust: {}, userBlacklist: [] }, (result) => {
      const isBlacklisted = result.userBlacklist.some(
      ...
      if (isBlacklisted) {
        ...
        chrome.tabs
          .sendMessage(tabId, {
            action: 'BLOCK_PAGE',
            data: { threat_type: 'Admin Force Blacklist', harm: 'High Risk Detected' },
          })
          .catch(() => {});
        return;
      }
      ...
      const isGlobal = result.globalTrusted.some(
        (d) => hostname === d || hostname.endsWith('.' + d)
      );
...
function performScan(tabId, url) {
  ...
  fetch(`${API_URL}/analyze`, {
    ...
    .then((data) => {
      ...
      chrome.storage.local.get({ ignoredDomains: [] }, (storage) => {
        ...
        if (data.risk_score > 75 && !isIgnored) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'USER_BYPASS') {
    try {
      ...
      console.error('Invalid URL format for bypass trust:', e);
    }
  }
  ...
  if (
    request.action === 'scan_link_heuristic' ||
    request.action === 'ANALYZE_URL' ||
    request.action === 'HOVER_SCAN'
  ) {
    try {
      const hostname = new URL(request.url).hostname;

      chrome.storage.local.get(
        { globalTrusted: [], userTrust: {}, userBlacklist: [] },
        (result) => {
          ...
          fetch(`${API_URL}/analyze`, {
            ...
            .catch((err) => {
              console.error('API Scan request failed:', err);
              sendResponse({ success: false, error: 'API_FAILED' });
            });
        }
      );
